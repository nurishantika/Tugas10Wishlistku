...
class AddEditNoteActivity : AppCompatActivity() {
    companion object {
        const val EXTRA_ID = "com.example.wishlistku.EXTRA_ID"
        const val EXTRA_JUDUL = "com.example.wishlistku.EXTRA_JUDUL"
        const val EXTRA_DESKRIPSI = "com.example.wishlistku.EXTRA_DESKRIPSI"
        const val EXTRA_PRIORITAS = "com.example.wishlistku.EXTRA_PRIORITAS"
    }
    ...
    private fun saveNote() {
        if (edit_text_title.text.toString().trim().isBlank() || edit_text_description.text.toString().trim().isBlank()) {
            Toast.makeText(this, "Wishlist kosong!", Toast.LENGTH_SHORT).show()
            return
        }
